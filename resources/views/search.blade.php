    ...
    <div class="card">
      ...
      <div class="todo">
        <form action="/find" method="POST" class="content-find">
          ...
          <select class = "select_tag" id = "tag_id" value = "{{$tagId}}" name = "tagId">
            <option value =""></option>
            @foreach ($tags as $tag)
              <option value="{{ $tag->id }}" @if($tag->id == '{{ $post->tagId }}') selected @endif>{{ $tag->tag_name }}</option>
            @endforeach
          </select>
            ...
            <form action="{{ route('todo.update', ['id' => $post->id]) }}" method="post">
              ...
              <td>
                <select class = "select_tag" name = "tagId">
                  @foreach ($tags as $tag)
                  <option value="{{ $tagId }}" @if($tags == '{{ $tag->id }}') selected @endif>{{ $tag->tag_name }}</option>
                  @endforeach
                </select>
              </td>
              <td>
